function FIGURE7d_shootingVectorChoiceFullrun(regenerate)
%
...
% for empty data)
plot_data = NaN(N_bins,size(iters_data,2),N_max);
...
plot_data = plot_data(:,plot_methods,:);
...
if failures_present
    ...
    for k = 1:size(fail_data,2)
        structfun(@(x) set( x(k,:), 'color', plot_colors(k,:), 'markeredgecolor', plot_colors(k,:), 'linewidth', 2.5), h2);
    end
