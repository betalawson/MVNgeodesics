function FIGURE7abc_shootingVectorChoice(regenerate)
%
...
legend_names = {'Euclidean', 'Taylor', 'Eigen', 'Transport'};

% Specify methods to plot (can be used to disclude methods)
plot_methods = [1,2,3];
...
%%% SELECTION OF METHODS TO PLOT
theta_data = theta_data(:,plot_methods);
KLimprovement_data = KLimprovement_data(:,plot_methods,:);
...
legend(legend_names(plot_methods), 'Fontsize', 24);
...
for k = 1:Nedges-1
    ...
    xtick_vals(end+1) = 0.5 * (L_edges(k) + L_edges(k+1)) - 0.025;      % Slight shift included to position text labels better
...
for k = 1:Nedges-1 
    xtick_labels{2*k} = ['\fontsize{16}\color{blue} (',num2str(N_bin(k)),')'];
end
